...
//     memset(num->digits, 0, BIGINT_MAX_DIGITS);

//     for (int i = 0; i < BIGINT_MAX_DIGITS && value > 0; ++i) {
//         int rem = value % 10;
//         value /= 10;
//         num->digits[i] = '0' + rem;
//     }
...
// // void bigint_init(BigInt* num, int value) {
// //     num->sign = (value < 0) ? -1 : 1; // Store sign
// //     if (value < 0) value = -value;    // Work with absolute value

// //     snprintf(num->digits, BIGINT_MAX_DIGITS, "%d", value);
// //     num->length = strlen(num->digits);

// //     // Reverse the digits for easier arithmetic
// //     for (int i = 0; i < num->length / 2; ++i) {
// //         char temp = num->digits[i];
// //         num->digits[i] = num->digits[num->length - i - 1];
// //         num->digits[num->length - i - 1] = temp;
// //     }
// // }
...
//     // Basic subtraction implementation (assuming a >= b)
//     int borrow = 0, i;
//     result->length = a->length;
//     for (i = 0; i < result->length; ++i) {
//         int diff = (a->digits[i] - '0') - (i < b->length ? b->digits[i] - '0' : 0) - borrow;
//         if (diff < 0) {
//             diff += 10;
//             borrow = 1;
//         } else {
